	...
	static createCards(objectArray, element) {
		...
		objectArray.forEach((object) => {
			...
			if (object.favourited) 
				document.getElementById(`fav-icon-path-${btnIdNum}`).classList.add('fav-icon-fill');

      if (object.discount) {
        const discountedPrice = Math.round((1 - object.discount / 100) * object.price);

        document.getElementById(`old-price-${btnIdNum}`).classList.add('old-price');
        document.getElementById(`discount-price-${btnIdNum}`).innerHTML = `$${discountedPrice}`;
        document.getElementById(`discount-price-${btnIdNum}`).style.display = "inline";
